...
|---|---|---|
| UP | Next Album | Volume Up |
| DOWN | Play / Pause | Volume Down |
| LEFT | Restart Track* | - |
| RIGHT | Next Track | - |
...
Teensyduino is an add-on for the Arduino IDE that adds support for Teensy boards.

**Arduino IDE 2.x (recommended):** Teensyduino is available as a board package through the Board Manager. In the Arduino IDE, go to Tools → Board → Boards Manager, search for "Teensy," and install the package by PJRC. No separate installer needed.

**Arduino IDE 1.x (legacy):** You need to run a separate installer:

1. Download the Teensyduino installer from https://www.pjrc.com/teensy/td_download.html
2. Run the installer and point it to your existing Arduino IDE installation directory
3. It will patch the IDE with Teensy support, libraries, and tools

You'll also need the **Teensy Loader** application, which handles uploading compiled code to the Teensy over USB. It typically comes bundled with the Teensyduino install, but can also be downloaded separately from the [PJRC site](https://www.pjrc.com/teensy/teensyduino.html).
### Required Libraries
...
1. **Arduino-Teensy-Codec-lib** (Frank Boesing)
   ...
   **Option A:** Download the ZIP from https://github.com/FrankBoesing/Arduino-Teensy-Codec-lib and install via Sketch → Include Library → Add .ZIP Library

   **Option B:** Download or clone the repo:

   ```
   git clone https://github.com/FrankBoesing/Arduino-Teensy-Codec-lib.git
   ```

   1. Rename the folder to `Arduino-Teensy-Codec-lib` (remove any `-master` suffix)
   2. Move it into your Arduino libraries folder:
