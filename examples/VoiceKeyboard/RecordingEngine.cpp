...
bool RecordingEngine::begin()
{
    ...
    DEBUG_PRINTF("RecordingEngine initialized, next seq: %u\n", nextSequenceNumber);
    return true;
}
...
bool RecordingEngine::createDirectories()
{
    ...
    if (!SD.exists("/RX"))
    {
        if (!SD.mkdir("/RX"))
        {
            DEBUG_PRINTLN("Failed to create RX directory");
            return false;
        }
    }
...
String RecordingEngine::generateFilename()
{
    // Format: /TX/MSG_NNNNN.opus
    char filename[48];
    snprintf(filename, sizeof(filename), "%s/MSG_%05u.opus",
             TX_DIR, nextSequenceNumber);
